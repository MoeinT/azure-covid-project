...
def transform_hospitals(df):
    return (
        df.withColumn("year", split("year_week", "-").getItem(0).cast(IntegerType())) \
            .withColumn("week", substring(split("year_week", "-").getItem(1), 2, 3).cast(IntegerType())) \
                .where(~col("indicator").contains("Weekly")) \
                    .groupBy("country", "date", "year", "week", "source").pivot("indicator").agg(sum("value"))
    )
...
if __name__ == "__main__":
    df_hospitals_processed = get_date(os.path.join("..", "data", "raw", "hospitals_admissions.csv"))
